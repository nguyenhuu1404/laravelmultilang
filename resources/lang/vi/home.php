<?php
return [
    'hometelsystem' => 'HỆ THỐNG HOMETEL',
    'contact' => 'Liên hệ',
    'fullname' => 'Họ và tên',
    'phone' => 'Điện thoại',
    'address' => 'Địa chỉ',
    'note' => 'Nội dung',
    'send' => 'Gửi',
    'contact_title' => 'DTJ Hometel là đơn vị đầu tiên cung cấp Căn hộ Hometel tại Hạ Long – hình thức lưu trú mới xuất hiện tại Việt Nam trong 1 năm trở lại đây.',
    'contact_info' => 'EMAIL:         nationhotel@example.com<br>
    ĐIỆN THOẠI:         +1 015 344 343 31<br>
    SKYPE:         hotelnationskype<br>
    Nhân viên kinh doanh: 0987.328.095 - 0986.314.470 - 0947.653.920<br>',
    'contact_success' => "Liên hệ thành công",
    'readmore' => 'Xem thêm',
    'footer_info' => 'Hometel Hạ Long: tòa nhà Greenbay Premium, Hoàng Quốc Việt, Hùng Thắng, Hạ Long, Quảng Ninh<br>
    Văn phòng Hà Nội: số 216 Hoàng Quốc Việt, Cầu Giấy, Hà Nội<br>
    Điện thoại: 024.7304.5999<br>
    Email: hometel@dtj.com.vn<br>
    Hotline: 0973.275.424<br>
    Nhân viên kinh doanh: 0987.328.095 - 0986.314.470 - 0947.653.920<br>',
    'bookingnow' => "Đặt phòng ngay",
    'booking' => "Đặt phòng",
    'checkin' => 'Ngày nhận phòng',
    'checkout' => 'Ngày trả phòng',
    'adult' => 'Người lớn',
    'children' => 'Trẻ em',
    'check' => 'Kiểm tra',
    'highlyapartment' => 'Căn hộ nổi bật',
    'service' => 'Dịch vụ tiện ích',
    'contactus' => 'Thông tin liên hệ',
    'topnews' => 'Tin tức nổi bật',
    'connectus' => 'Kết nối với chúng tôi',
    'copyright' => 'Bản quyền @2019 DTJ HOMETEL – DTJ GROUP',
    'apartment' => 'Danh sách căn hộ',
    'standard' => 'Tiêu chuẩn phòng',
    'description' => 'Mô tả',
    'convenient' => 'Tiện nghi căn hộ',
    'policy' => 'Chính sách',
    'relatedapartment' => 'Căn hộ liên quan ',
    'yourroom' => 'Đặt phòng của bạn ',
    'photo' => 'Hình ảnh',
    'relatednews' => 'Tin tức liên quan ',
    'hometelhalong' => 'Hometel Hạ Long',
    'cnhalong' => 'Chi nhánh Hạ Long',
    'news' => 'Tin tức',
    'relatepost' => 'Bài viết liên quan ',
    'signup' => 'Đăng ký để nhận được các thông tin ưu đãi mới nhất của DTJ Hometel',
    'pricefrom' => 'Giá từ',
    'night' => 'đêm',
    'change' => 'Thay đổi',
    'cancel' => 'Hủy',
    'edit' => 'Chỉnh sử đặt chỗ',
    'lastname' => 'Tên',
    'firstname' => 'Họ',
    'payment' => 'Thanh toán',
    'pay' => 'Thanh toán',
    'apm'   => 'Căn hộ',
    'total' => 'Tổng tiền',
    'paycontent' => ' Anh/ chị vui lòng chuyển khoản số tiền cọc 50% tổng tiền vào TK thông tin dưới đây trong vòng 24h kể từ khi đặt phòng để được giữ phòng:<br/>
    <b>STK:</b> 21710000303133 <br/>
    <b>Ngân hàng:</b> BIDV <br/>
    <b>Chi nhánh:</b> Từ Liêm<br/>
    <b>Chủ Tài Khoản:</b> Công Ty Cổ Phần Đầu Tư và Phân Phối DTJ <br/>
    <b>Nội Dung:</b> Họ và tên + SĐT<br/><br/>',
    'paythanks' => 'Quý khách đã hoàn tất các thủ tục đặt phòng. Nhân viên của DTJ Hometel sẽ liên hệ lại với quý khách trong thời gian sớm nhất để xác nhận thông tin. Hoặc quý khách có thể liên hệ trực tiếp để được tư vấn.<br/>
    Hotline: <span class="text-white">0973.275.424</span><br/>
    Nhân viên kinh doanh: <span class="text-white">0987.328.095 - 0986.314.470 - 0947.653.920</span><br/><br/>
    Trân trọng cảm ơn!',
    'chooseroom' => 'Chọn phòng',
    'chooseapartment' => 'Chọn căn hộ',
    'confirm' => 'Xác nhận',
    'peri' => 'Thông tin cá nhân',
    'bc' => 'Đặt phòng hoàn tất',
    'choosedate' => 'Chọn ngày',
    'bookingpage' => 'Trang đặt phòng',
    'emailsuccess' => 'Đăng kí nhận email thành công',
    'search' => 'Tìm kiếm',
    'noresult' => 'Không có kết quả nào',
    'services' => 'Dịch vụ'

];
